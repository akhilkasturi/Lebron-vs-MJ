...
let currentPlayer = "lebron";
const dataFiles = {
    lebron: "lebron_playoff_data.json",
    jordan: "jordan_playoff_data.json"
};

function loadData(player) {
    d3.json(dataFiles[player]).then(data => {
                ...
                visibleRounds.forEach(round => {
                    ...
                    group.append("rect")
                        ...
                        .on("mouseover", function () {
                         ...
                         SRS Rank: ${round.srs_rank} / ${currentPlayer === "lebron" ? 30 : 29}<br>
...
d3.select("#toggle-player").on("click", function () {
    currentPlayer = currentPlayer === "lebron" ? "jordan" : "lebron";
    d3.select(this).text(currentPlayer === "lebron" ? "LeBron" : "Jordan");
    svg.selectAll("g.bar").remove();
    loadData(currentPlayer);
});
